...
inline werkzeugkiste::geometry::Vec<Tp, Dim> VecFromTupleOrList(
    const pybind11::tuple &tpl, std::string_view py_type,
...
werkzeugkiste::geometry::Vec<Tp, Dim> VecFromList(
    const pybind11::list &lst, std::string_view module_name) {
  return VecFromTupleOrList<Tp, Dim>(lst.cast<pybind11::tuple>(), "list",
                                     module_name);
}

template <typename Tp, std::size_t Dim>
werkzeugkiste::geometry::Vec<Tp, Dim> VecFromTuple(
    const pybind11::tuple &tpl, std::string_view module_name) {
...
/// from VecFromArray()
template <typename ATp, typename VTp, std::size_t Dim>
inline werkzeugkiste::geometry::Vec<VTp, Dim> VecFromArrayT(
    const pybind11::array &arr, std::string_view module_name) {
  ...
  const int col_stride = (arr.ndim() > 1) ? (arr.strides(1) / sizeof(ATp)) : 1;
  const int channel_stride =
      (arr.ndim() == 3) ? (arr.strides(2) / sizeof(ATp)) : 1;
...
/// DIM-dimensional Vec<>
template <typename Tp, std::size_t Dim>
inline werkzeugkiste::geometry::Vec<Tp, Dim> VecFromArray(
    const pybind11::array &arr, std::string_view module_name) {
  using VC = werkzeugkiste::geometry::Vec<Tp, Dim>;
  ...
  if (pybind11::isinstance<pybind11::array_t<uint8_t>>(arr)) {
    ...
    s += ", \"";
    const pybind11::list dt_descr =
        pybind11::cast<pybind11::list>(dtype.attr("descr"));
...
werkzeugkiste::geometry::Vec<Tp, Dim> VecFromPyObject(
    const pybind11::object &object, std::string_view module_name) {
  using VC = werkzeugkiste::geometry::Vec<Tp, Dim>;

  if (pybind11::isinstance<pybind11::tuple>(object)) {
    return VecFromTupleOrList<Tp, Dim>(object.cast<pybind11::tuple>(), "tuple",
                                       module_name);
  }

  if (pybind11::isinstance<pybind11::list>(object)) {
    return VecFromTupleOrList<Tp, Dim>(object.cast<pybind11::tuple>(), "list",
                                       module_name);
  }

  if (pybind11::isinstance<pybind11::array>(object)) {
    return VecFromArray<Tp, Dim>(object.cast<pybind11::array>(), module_name);
  }
...
template <>
const char *PythonVecTypeName<double>(bool rst) {
  if (rst) {
    return ":class:`float`";
  } else {
    return "float:";
...
template <>
const char *PythonVecTypeName<int32_t>(bool rst) {
  if (rst) {
    return ":class:`int`";
  } else {
    return "int:";
...
pybind11::buffer_info VecInfo(werkzeugkiste::geometry::Vec<Tp, Dim> &vec) {
  return pybind11::buffer_info(
      ...
      pybind11::format_descriptor<Tp>::format(),  // Python struct-style format
                                                  // descriptor
...
template <class V>
inline void RegisterVectorAdditionSubtraction(pybind11::class_<V> &vec,
                                              std::string_view module_name) {
  ...
  const std::string fqn_int32 =
      std::string(module_name) + "." + V_int32::TypeName();

  //---------------------------------------------------------------------------
  // Addition "V + X"

  std::ostringstream doc;
  doc << "Element-wise addition: :class:`~" << fqn_type << "` = :class:`~"
      << fqn_type << "` + :class:`int`.";
  vec.def(
      "__add__",
      ...
      doc.str().c_str(), pybind11::arg("scalar"));
  ...
  vec.def(
      ...
      doc.str().c_str(), pybind11::arg("lhs"));

  // Vector + Vector (of the same type)
  std::ostringstream().swap(doc);
  doc << "Element-wise addition: :class:`~" << fqn_type << "` = :class:`~"
      << fqn_type << "` + :class:`~" << fqn_type << "`.";
  vec.def(
      "__add__", [](const V &self, const V &rhs) -> V { return self + rhs; },
      doc.str().c_str(), pybind11::arg("rhs"));

  if constexpr (std::is_floating_point_v<value_type>) {
    // Allow implicit upcasting of int32 vectors
    std::ostringstream().swap(doc);
    doc << "Element-wise addition: :class:`~" << fqn_type << "` = :class:`~"
        << fqn_type << "` + :class:`~" << fqn_int32 << "`.";
    vec.def(
        "__add__",
    ...
    std::ostringstream().swap(doc);
    doc << "Element-wise addition: :class:`~" << fqn_type << "` = :class:`~"
        << fqn_type << "` + :class:`float`.";
    vec.def(
        "__add__", [](const V &self, double rhs) -> V { return self + rhs; },
        doc.str().c_str(), pybind11::arg("rhs"));
    ...
    vec.def(
        "__radd__", [](const V &self, double lhs) -> V { return self + lhs; },
        doc.str().c_str(), pybind11::arg("lhs"));
  }
  ...
  vec.def(
      "__iadd__",
      [](V &self, const V &rhs) -> V & { return self.AddVector(rhs); },
      doc.str().c_str(), pybind11::arg("rhs"));
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    ...
    vec.def(
        "__iadd__",
        [](V &self, double rhs) -> V & { return self.AddScalar(rhs); },
        doc.str().c_str(), pybind11::arg("rhs"));
  }
  ...
  std::ostringstream().swap(doc);
  doc << "Element-wise subtraction: :class:`~" << fqn_type << "` = :class:`~"
      << fqn_type << "` - :class:`int`.";
  vec.def(
      "__sub__",
      [](const V &self, int32_t rhs) -> V {
  ...
  doc << "Element-wise subtraction: :class:`~" << fqn_type << "` = :class:`~"
      << fqn_type << "` - :class:`~" << fqn_type << "`.";
  vec.def(
      "__sub__", [](const V &self, const V &rhs) -> V { return self - rhs; },
      doc.str().c_str(), pybind11::arg("rhs"));
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    ...
    doc << "Element-wise subtraction: :class:`~" << fqn_type << "` = :class:`~"
        << fqn_type << "` - :class:`float`.";
    vec.def(
        "__sub__", [](const V &self, double rhs) -> V { return self - rhs; },
        doc.str().c_str(), pybind11::arg("rhs"));
    ...
    vec.def(
        "__rsub__",
        [](const V &self, double lhs) -> V { return V::All(lhs) - self; },
        doc.str().c_str(), pybind11::arg("lhs"));
  }

  // TODO check if flaky warnings still need to be suppressed (switched to
  // explicit lambda) #ifdef __clang__
...
template <class V>
inline void RegisterVectorMultiplication(pybind11::class_<V> &vec,
                                         std::string_view module_name) {
  using value_type = typename V::value_type;
  using V_int32 = werkzeugkiste::geometry::Vec<value_type, V::ndim>;
  ...
  // Vec * int
  std::ostringstream doc;
  doc << "Element-wise multiplication: :class:`~" << fqn_type << "` = :class:`~"
      << fqn_type << "` * :class:`int`.";
  vec.def(
      "__mul__",
      [](const V &self, int32_t rhs) -> V {
  ...
  doc << "Element-wise multiplication: :class:`~" << fqn_type << "` = :class:`~"
      << fqn_type << "` * :class:`~" << fqn_type << "`.";
  vec.def(
      "__mul__", [](const V &self, const V &rhs) -> V { return self * rhs; },
      doc.str().c_str(), pybind11::arg("rhs"));
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    ...
    std::ostringstream().swap(doc);
    doc << "Element-wise multiplication: :class:`~" << fqn_type
        << "` = :class:`~" << fqn_type << "` * :class:`~" << fqn_int32 << "`.";
    vec.def(
        "__mul__",
        [](const V &self, const V_int32 &rhs) -> V {
          V dbl = rhs.ToDouble();
    ...
    std::ostringstream().swap(doc);
    doc << "Element-wise multiplication: :class:`~" << fqn_type
        << "` = :class:`~" << fqn_int32 << "` * :class:`~" << fqn_type << "`.";
    vec.def(
        "__rmul__",
        [](const V &self, const V_int32 &lhs) -> V {
          V dbl = lhs.ToDouble();
    ...
    vec.def(
        "__mul__", [](const V &self, double rhs) -> V { return self * rhs; },
        doc.str().c_str(), pybind11::arg("rhs"));
    ...
    doc << "Element-wise multiplication: :class:`~" << fqn_type
        << "` = :class:`float` * :class:`~" << fqn_type << "`.";
    vec.def(
        "__rmul__", [](const V &self, double lhs) -> V { return self * lhs; },
        doc.str().c_str(), pybind11::arg("lhs"));
  }
...
template <class V>
inline void RegisterVectorDivision(pybind11::class_<V> &vec,
                                   std::string_view module_name) {
  using value_type = typename V::value_type;
  using V_int32 = werkzeugkiste::geometry::Vec<value_type, V::ndim>;
  ...
  const std::string fqn_int32 =
      std::string(module_name) + "." + V_int32::TypeName();
  const std::string fqn_double =
      std::string(module_name) + "." + V_double::TypeName();

  std::string prefix{};
  if constexpr (std::is_integral_v<value_type>) {
    prefix =
        "Element-wise division with **implicit type conversion**. Returns\n"
        "a **double-precision** :class:`~" +
        fqn_double + "` = ";
  } else {
  ...
  std::ostringstream().swap(doc);
  doc << prefix << ":class:`~" << fqn_type << "` / :class:`~" << fqn_type
      << "`.";
  vec.def(
      "__truediv__",
...
void RegisterVector(pybind11::module &m) {
  ...
  pybind11::class_<VC> vec_cls(m, VC::TypeName().c_str(),
                               pybind11::buffer_protocol(), doc.str().c_str());
  ...
  if constexpr (std::is_same_v<VC, VC_double>) {
    ...
    std::ostringstream().swap(doc);
    doc << "Creates a double-precision vector from its 32-bit integral "
           "counterpart.";
    vec_cls.def(
        pybind11::init<>([](const VC_int32 &vec) { return vec.ToDouble(); }),
        doc.str().c_str(), pybind11::arg("vec_int"));
  ...
  doc << "Creates a vector where each coordinate is set to the given scalar."
         "\n\n**Corresponding C++ API:** ``"
      << module_name << "::" << VC::TypeName() << "::All``.";
  vec_cls.def_static("all", &VC::All, doc.str().c_str(),
                     pybind11::arg("value"));

  std::ostringstream().swap(doc);
  doc << "Creates a copy of the given ``" << VC::TypeName() << "``.";
  vec_cls.def(pybind11::init<const VC &>(), doc.str().c_str(),
              pybind11::arg("vec"));

  std::ostringstream().swap(doc);
  doc << "Creates a vector from a " << Dim << "-element :class:`tuple`.";
  vec_cls.def(pybind11::init<>([module_name](const pybind11::tuple &tpl) {
  ...
  vec_cls.def(pybind11::init<>([module_name](const pybind11::list &lst) {
  ...
  vec_cls.def(pybind11::init<>([module_name](const pybind11::array &arr) {
                return VecFromArray<Tp, Dim>(arr, module_name);
              }),
              doc.str().c_str(), pybind11::arg("arr"));
  ...
  vec_cls.def_property_readonly(
      "dtype",
      [](const VC & /* self */) {
        return pybind11::dtype(pybind11::format_descriptor<Tp>::format());
      },
  ...
  if constexpr (std::is_integral_v<Tp>) {
    doc << "Returns a copy of this vector as :class:`~" << module_name << '.'
        << VC_int32::TypeName() << "`\n\n**Corresponding C++ API:** ``"
        << module_name << "::" << VC::TypeName() << "::ToInteger``.";
  } else {
  ...
  vec_cls.def("as_int", &VC::ToInteger, doc.str().c_str());
  ...
  if constexpr (std::is_integral_v<Tp>) {
    doc << "Returns a :class:`~" << module_name << '.' << VC_int32::TypeName()
        << "` as the result\nof **explicitly casting** the values."
           "\n\n**Corresponding C++ API:** ``"
        << module_name << "::" << VC::TypeName() << "::ToDouble``.";
  } else {
    doc << "Returns a copy of this vector as :class:`~" << module_name << '.'
        << VC_double::TypeName() << "`\n\n**Corresponding C++ API:** ``"
        << module_name << "::" << VC::TypeName() << "::ToDouble``.";
  }
  vec_cls.def("as_float", &VC::ToDouble, doc.str().c_str());

  // TODO follow best practices: https://stackoverflow.com/a/2626364/400948
  // TODO test eval(repr(x)) == x
  vec_cls.def("__repr__", [module_name](const VC &v) {
    std::ostringstream s;
    s << module_name << '.'
      << v.ToString(true);  // TODO add param to werkzeugkiste.tostring
                            // (shorten_precision)!
    return s.str();
  });

  // TODO pretty print
  vec_cls.def("__str__", [](const VC &v) { return v.ToString(false); });

  std::ostringstream().swap(doc);
  doc << PythonVecTypeName<Tp>(false)
      << " Read-write access to the first dimension, *i.e.* same as "
         "``vec[0]``.";
  vec_cls.def_property("x", static_cast<const Tp &(VC::*)() const>(&VC::X),
                       &VC::SetX, doc.str().c_str());

  if constexpr (Dim >= 2) {
    std::ostringstream().swap(doc);
    doc << PythonVecTypeName<Tp>(false)
        << " Read-write access to the second dimension, *i.e.* same as "
           "``vec[1]``.";
    vec_cls.def_property(
        "y", [](const VC &self) -> Tp { return self.Y(); },
  ...
  vec_cls
      .def_property_readonly(
          "ndim", [](const VC &) { return Dim; }, doc.str().c_str())
      .def(
          "__setitem__", [](VC &self, int index, Tp v) { self[index] = v; },
  ...
  doc << "Returns a deep copy.\n\n"
      ...
      << module_name << "::" << VC::TypeName() << "``.";
  vec_cls.def(
      "copy", [](const VC &self) { return VC(self); }, doc.str().c_str());

  std::ostringstream().swap(doc);
  doc << "Returns the `homogeneous representation "
         "<https://en.wikipedia.org/wiki/Homogeneous_coordinates>`__ "
         "of this vector, *i.e.* a "
      << (Dim + 1)
      << "-element `tuple`, where the additional dimension is "
         "set to 1.";
  vec_cls.def(
      "homogeneous",
      [](const VC &self) {
  ...
  vec_cls.def("length_squared", &VC::LengthSquared,
  ...
  std::ostringstream().swap(doc);
  doc << "Returns the Euclidean distance between ``self`` and the other "
         ":class:`~"
      << module_name << '.' << VC::TypeName() << "`.";
  ...
  std::ostringstream().swap(doc);
  doc << "Returns the Manhattan distance between ``self`` and the other "
         ":class:`~"
      << module_name << '.' << VC::TypeName() << "`.";
  ...
  vec_cls.def("direction_vector", &VC::DirectionVector, doc.str().c_str(),
              pybind11::arg("other"));

  std::ostringstream().swap(doc);
  doc << "Returns the corresponding unit vector as :class:`~" << module_name
      << '.' << VC_double::TypeName() << "`.";
  ...
  vec_cls.def(pybind11::pickle(&VecToTuple<Tp, Dim>,
  ...
  if constexpr (std::is_integral_v<Tp>) {
    vec_cls.def(
        "__eq__", [](const VC &a, const VC &b) -> bool { return a == b; },
        "Checks for equality.", pybind11::arg("other"));

    vec_cls.def(
        "__ne__", [](const VC &a, const VC &b) -> bool { return a != b; },
        "Checks for inequality.", pybind11::arg("other"));
  } else {
    std::ostringstream().swap(doc);
    doc << "Returns ``True`` if the two vectors are *approximately equal*.\n\n"
           ...
           ":math:`x = (x_i)_{i=1}^"
        << Dim << "` and :math:`y = (y_i)_{i=1}^" << Dim
        << "` are considered\n*approximately equal* if for all their values\n\n"
           ":math:`|x_i - y_i| \\leq \\epsilon_{\\text{rel}} * \\max(|x_i|, "
           "|y_i|) "
           "\\quad\\text{or}\\quad |x_i - y_i| \\leq "
           "\\epsilon_{\\text{abs}}`,\n\nwhere "
           ":math:`\\epsilon_{\\text{rel}} = 1e-9` and "
           ":math:`\\epsilon_{\\text{abs}} = 1e-12`."
           "\n\n**Corresponding C++ API:** ``"
        << module_name << "::" << VC::TypeName()
        << "::operator==`` and ``::IsClose``";
    vec_cls.def(
        "__eq__",
    ...
    doc << "Returns ``True`` if the two vectors differ *sufficiently*.\n\n"
           ...
           ":meth:`~"
        << module_name << '.' << VC::TypeName() << ".__eq__` for details.";
    vec_cls.def(
        "__ne__", [](const VC &a, const VC &b) -> bool { return a != b; },
        doc.str().c_str(), pybind11::arg("other"));
  }
  ...
  vec_cls.def("__abs__", &VC::Absolute, doc.str().c_str());
  ...
  if constexpr (Dim == 2) {
    ...
    vec_cls.def(pybind11::init<Tp, Tp>(), doc.str().c_str(), pybind11::arg("x"),
                pybind11::arg("y"));
    ...
    vec_cls.def_property(
        "width", [](const VC &self) -> Tp { return self.Width(); },
    ...
    doc << PythonVecTypeName<Tp>(false)
        << " Provides alternative read-write access to\n"
           ...
           "  for 2-dimensional vectors.";
    vec_cls.def_property(
        "height", [](const VC &self) -> Tp { return self.Height(); },
  ...
  if constexpr (Dim > 2) {
    std::ostringstream().swap(doc);
    doc << PythonVecTypeName<Tp>(false)
        << " Read-write access to the third dimension, *i.e.* same as "
           "``vec[2]``.";
    vec_cls.def_property(
        "z", [](const VC &self) -> Tp { return self.Z(); },
  ...
  if constexpr (Dim > 3) {
    if constexpr (Dim == 4) {
      std::ostringstream().swap(doc);
      doc << "Explicit initialization from " << Dim << " separate values.";
      vec_cls.def(pybind11::init<Tp, Tp, Tp, Tp>(), doc.str().c_str(),
                  pybind11::arg("x"), pybind11::arg("y"), pybind11::arg("z"),
                  pybind11::arg("w"));
    }
    ...
    doc << PythonVecTypeName<Tp>(false)
        << " Read-write access to the fourth dimension, "
           "*i.e.*  same as ``vec[3]``.";
    vec_cls.def_property("w", static_cast<const Tp &(VC::*)() const>(&VC::W),
